...
impl BVHNode
{
    pub fn new(left: Arc<dyn Hittable + Send + Sync>, right: Arc<dyn Hittable + Send + Sync>, box_: AABB) -> BVHNode {
        BVHNode { left, right, box_ }
    }

    pub fn from_list(list: HittableList, time0: f32, time1: f32) -> Arc<dyn Hittable + Send + Sync> {
        ...
        let left = if objects.len() == 1 {
            ...
            BVHNode::from_list(HittableList { objects: objects[0..objects.len() / 2].to_vec() }, time0, time1)
        };
        let right = if objects.len() == 1 {
            objects[0].clone_dyn()
        } else {
            BVHNode::from_list(HittableList { objects: objects[objects.len() / 2..].to_vec() }, time0, time1)
        };
        let box_left = left.bounding_box(time0, time1).unwrap();
        let box_right = right.bounding_box(time0, time1).unwrap();
        Arc::new(BVHNode::new(left, right, box_left.surrounding_box(box_right)))
    }
}
...
impl Hittable for BVHNode
{
    ...
    fn clone_dyn(&self) -> Arc<dyn Hittable + Send + Sync> {
        Arc::new(BVHNode::new(self.left.clone(), self.right.clone(), self.box_))
    }
}